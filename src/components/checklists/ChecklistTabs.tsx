            ...
            <Table>
              ...
              <TableBody>
                {checklists
                  ...
                  const dFactorCount = template.questions.filter(q => (q as DiscQuestion).targetFactor === "D").length;
                  const iFactorCount = template.questions.filter(q => (q as DiscQuestion).targetFactor === "I").length;
                  const sFactorCount = template.questions.filter(q => (q as DiscQuestion).targetFactor === "S").length;
                  const cFactorCount = template.questions.filter(q => (q as DiscQuestion).targetFactor === "C").length;
                ...
                {checklists.map((template) => {
                  ...
                  const categories = template.type === "psicossocial"
                    ? Array.from(new Set(template.questions.map(q => {
                        // Certifique-se de que estamos tratando uma pergunta psicossocial
                        if ('category' in q) {
                          return (q as PsicossocialQuestion).category;
                        }
                        // Caso contrário, se a pergunta tem targetFactor, use isso
                        else if ('targetFactor' in q) {
                          return (q as DiscQuestion).targetFactor;
                        }
                  ...
                  const dFactorCount = template.type === "disc" ? 
                    template.questions.filter(q => 'targetFactor' in q && (q as DiscQuestion).targetFactor === "D").length : 0;
                  const iFactorCount = template.type === "disc" ? 
                    template.questions.filter(q => 'targetFactor' in q && (q as DiscQuestion).targetFactor === "I").length : 0;
                  const sFactorCount = template.type === "disc" ? 
                    template.questions.filter(q => 'targetFactor' in q && (q as DiscQuestion).targetFactor === "S").length : 0;
                  const cFactorCount = template.type === "disc" ? 
                    template.questions.filter(q => 'targetFactor' in q && (q as DiscQuestion).targetFactor === "C").length : 0;
                      ...
                      <TableCell className="text-right">
                        <div className="flex justify-end gap-2">
                          ...
                          <Button 
                            variant="outline" 
                            size="sm" 
                            onClick={() => onStartAssessment(template)}
                          >
                            Iniciar Avaliação
                          </Button>
                        </div>
