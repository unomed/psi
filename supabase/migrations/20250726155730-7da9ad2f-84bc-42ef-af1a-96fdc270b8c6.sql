...
-- Fix authenticate_employee function to work with proper password hashing
...
BEGIN
  ...
  -- Get employee data (normalize CPF comparison)
  ...
  IF v_employee.password_hash IS NULL THEN
    ...
    -- Validate using last 4 digits of clean CPF
  ...
  IF v_is_valid THEN
    ...
    VALUES (v_employee.id, COALESCE(v_employee.password_hash, crypt(p_password, gen_salt('bf', 12))), gen_salt('bf'), 0, NOW())
