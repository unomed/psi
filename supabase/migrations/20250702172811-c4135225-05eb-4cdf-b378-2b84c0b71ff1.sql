-- FASE 2: Configurações Psicossociais - Criar tabelas para sistema de análise de risco
...
-- 4. Jobs de processamento psicossocial (já existe, mas vou melhorar)
...
CREATE TRIGGER update_psychosocial_category_weights_updated_at
  BEFORE UPDATE ON psychosocial_category_weights
  FOR EACH ROW EXECUTE FUNCTION update_psychosocial_updated_at();

CREATE TRIGGER update_psychosocial_risk_config_updated_at
  BEFORE UPDATE ON psychosocial_risk_config
  FOR EACH ROW EXECUTE FUNCTION update_psychosocial_updated_at();
